...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testFilter() {
        ...
        List<String> actual = input.filter(a -> a.startsWith("b")).collect(Collectors.toList());
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(a -> a.replaceAll("\\d+", "")).distinct().collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input
                .filter(a -> Character.getNumericValue(a.charAt(a.length() -1)) % 2 == 0).distinct()
                .reduce("", (finalString, iterator) -> finalString+iterator,(a,b) -> a+b);
    ...
    public void testCountByDate() {
        ...
        long actual = input.filter(a->a.timestamp.isBefore(instant)).count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountWhitelisted() {
        ...
        long actual = input.filter(a->whitelist.contains(a.user)).count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(a->!a.user.toLowerCase().contains("bot")).count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesInFebruary() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual =input.filter(a->a.timestamp.atZone(ZoneId.systemDefault()).query(LocalDateTime::from).getMonth()
        .equals(Month.FEBRUARY)).count();
        int expected = 9;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesByMonth() {
        Stream<Revision> input = getRevisions("soup04.json");
        Map<Month, Long> actual = input.sorted(Comparator.comparing(r->r.timestamp))
                .map(r->r.timestamp.atZone(ZoneId.systemDefault()).getMonth())
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = input.map(r->r.timestamp.atZone(ZoneId.systemDefault()).getMonth())
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
                .entrySet().stream()
